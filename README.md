...
VPN接続状態は、ipwho.is が返す出口IPの登録組織名（`connection.isp` / `org`）を `VPN_ORG_PATTERNS` と照合し、一致すれば事業者名まで出す（例：`Mullvad VPN AB` → Mullvad VPN）。汎用ホスティングを出口にしている事業者は `DATACENTER_ORG_RE` に当たるので「データセンター経由」までしか分からない。どちらにも当たらなければ一般ISP回線と見なす。新しい事業者を足すときはこの2つの定数を編集する。
