...
- VPN接続状態（推定）— 出口IPの登録組織名から事業者を推定する
...
診断リクエストはすべて `bust()` でURLに時刻を付ける。`cache:'no-store'` はブラウザキャッシュにしか効かず、CDNやAPI側のエッジキャッシュには効かないため、回線を切り替えた直後に前の回線の結果が返ることがある。
...
DNSリーク検査は `https://edns.ip-api.com/json` を使い、**実際に名前解決を行ったリゾルバ**のIPと所在地を取る。これを出口IPの国と比べ、一致しなければISPのDNSに漏れている疑いとして扱う。dnsleaktest.com の拡張テストのように全リゾルバを列挙するには権威DNSサーバー側でログを取る必要があり、静的ページだけでは実施できない（旧実装は存在しない `dnsleaktest.com/api/v1/nameservers` を叩いていて常に失敗していた）。網羅的な確認はチェックリストの項目から dnsleaktest.com を直接開く運用にしている。
